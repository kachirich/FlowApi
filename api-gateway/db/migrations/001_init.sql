-- =============================================================================
-- Migration 001: Full schema snapshot — idempotent baseline.
-- Captures everything currently created by initializeDatabase() in connection.js
-- plus the guest_sessions table.  Safe to re-run; every statement uses
...
-- 2. Users (full column set before the split in 002)
...
-- Additional users columns added over time
...
-- flow_id FK deferred until after the flows table is created below
...
-- Dynamic Dispatcher — rename column if old name still exists
DO $$ BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.columns
             WHERE table_name='webhook_keys' AND column_name='destination_url') THEN
...
INSERT INTO gateway_counters (key, value) VALUES ('bots_blocked', 0) ON CONFLICT (key) DO NOTHING;
...
-- Now safe to add the flow_id FK to api_keys (flows table exists above)
ALTER TABLE api_keys ADD COLUMN IF NOT EXISTS flow_id UUID
  REFERENCES flows(id) ON DELETE SET NULL;

-- 12. Destination balances (prepaid credit metering)
...
CREATE INDEX IF NOT EXISTS idx_dest_balances_user ON destination_balances (user_id);
...
CREATE INDEX IF NOT EXISTS idx_balance_tx_dest ON balance_transactions (destination_id, created_at DESC);

-- 13. Guest sessions (live tech demo)
